...
(function($){
	ojo.extend({
		...
		spacegallery: {
			...
			positionImages: function(el) {
				var top = 0;
				ojo.spacegallery.animated = false;
				$(el)
					.find('a')
			...
			next: function(e) {
				if (ojo.spacegallery.animated === false) {
					ojo.spacegallery.animated = true;
						...
						.animate({
							...
							complete: function() {
								$(el)
									.find('img:last')
									.prependTo(el);
								ojo.spacegallery.positionImages(el);
			...
			//constructor
			init: function(opt) {
				opt = $.extend({}, ojo.spacegallery.defaults, opt||{});
				return this.each(function(){
					...
					if ($(el).is('.spacegallery')) {
						$('<a href="#"></a>')
							...
							.bind('click', ojo.spacegallery.next);
	...
	$.fn.extend({
		 ...
		 */
		spacegallery: ojo.spacegallery.init
	});
	$.extend($.easing,{
